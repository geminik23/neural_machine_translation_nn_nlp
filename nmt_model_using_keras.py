...
class NmtModel(object):
    ...
    def build(self):
        ...
        ##### encoder
        ##
        embedding_source = Embedding(self.vocab_source_size, self.embedding_size, mask_zero=True)
        embedding_target = Embedding(self.vocab_target_size, self.embedding_size, mask_zero=True)

        source_words_embeddings = Dropout(self.embedding_dropout_rate)(embedding_source(source_words))
        target_words_embeddings = Dropout(self.embedding_dropout_rate)(embedding_target(target_words))
    ...
    def eval(self, dataset):
        ...
        return source_words, candidates, references
...
class AttentionLayer(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        # permute dimensions
        decoder_outputs_ = K.permute_dimensions(decoder_outputs, (0, 2, 1))
        # softmax at axis 1 : max_source_sent_len
        luong_score = K.softmax(K.batch_dot(encoder_outputs, decoder_outputs_), axis=1)

        # for element-wise multiplication, expand the dimensions and sum the max_source_sent_len (axis=1)
        encoder_vector = K.sum(
                K.expand_dims(encoder_outputs, axis=2) * K.expand_dims(luong_score, axis=-1),
                axis=1)
...
def print_rand_samples(model, data):
    source_words, candidates, references = model.eval(test_data)
    idx = np.random.randint(len(source_words))
    print("a sample reference at index {} : {}".format(idx, " ".join(references[idx][0])))
...
if __name__ == '__main__':
    ...
    print_rand_samples(model, test_data)
